...
#include "HunterPlayerInput.h"
#include "Performance/LatencyMarkerModule.h"
#include "Settings/HunterSettingsLocal.h"

UHunterPlayerInput::UHunterPlayerInput()
	: Super()
{
...
bool UHunterPlayerInput::InputKey(const FInputKeyEventArgs& Params)
{
	const bool bResult = Super::InputKey(Params);

	// Note: Since Hunter is only going to support the "Reflex" plugin to handle latency markers,
...
void UHunterPlayerInput::ProcessInputEventForLatencyMarker(const FInputKeyEventArgs& Params)
{
...
void UHunterPlayerInput::BindToLatencyMarkerSettingChange()
{
...
void UHunterPlayerInput::UnbindLatencyMarkerSettingChangeListener()
{
	UHunterSettingsLocal* Settings = UHunterSettingsLocal::Get();
	if (!Settings)
	{
...
void UHunterPlayerInput::HandleLatencyMarkerSettingChanged()
{
	// Make sure that we only ever get this callback on platforms which support latency markers
	ensure(UHunterSettingsLocal::DoesPlatformSupportLatencyMarkers());
