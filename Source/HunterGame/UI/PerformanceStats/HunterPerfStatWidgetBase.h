...
#include "HunterPerfStatWidgetBase.generated.h"
...
class SHunterLatencyGraph : public SLeafWidget
{
public:
	/** Begin the arguments for this slate widget */
	SLATE_BEGIN_ARGS(SHunterLatencyGraph)
		: _DesiredSize(150, 50),
...
class UHunterPerfStatGraph : public UUserWidget
{
	GENERATED_BODY()

public:
	UHunterPerfStatGraph(const FObjectInitializer& ObjectInitializer);
...
protected:
	...
	TSharedPtr<SHunterLatencyGraph> SlateLatencyGraph;
};
 ...
 */
 UCLASS(Abstract)
class UHunterPerfStatWidgetBase : public UCommonUserWidget
{
public:
	GENERATED_BODY()
...
public:
	...
	EHunterDisplayablePerformanceStat GetStatToDisplay() const
	{
...
protected:
 	...
 	UHunterPerformanceStatSubsystem* GetStatSubsystem();
	...
	UPROPERTY(BlueprintReadWrite, meta=(BindWidget, OptionalWidget=true))
 	TObjectPtr<UHunterPerfStatGraph> PerfStatGraph;
 	
	// Cached subsystem pointer
	UPROPERTY(Transient)
	TObjectPtr<UHunterPerformanceStatSubsystem> CachedStatSubsystem;
