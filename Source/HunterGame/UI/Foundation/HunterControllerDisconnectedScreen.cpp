...
#include "HunterControllerDisconnectedScreen.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterControllerDisconnectedScreen)
...
UHunterControllerDisconnectedScreen::UHunterControllerDisconnectedScreen(const FObjectInitializer& ObjectInitializer)
...
void UHunterControllerDisconnectedScreen::NativeOnActivated()
{
	Super::NativeOnActivated();

	if (!HBox_SwitchUser)
	{
		UE_LOG(LogHunter, Error, TEXT("Unable to find HBox_SwitchUser on Widget %s"), *GetNameSafe(this));
		return;
	}

	if (!Button_ChangeUser)
	{
		UE_LOG(LogHunter, Error, TEXT("Unable to find Button_ChangeUser on Widget %s"), *GetNameSafe(this));
		return;
	}
...
bool UHunterControllerDisconnectedScreen::ShouldDisplayChangeUserButton() const
{
...
void UHunterControllerDisconnectedScreen::HandleChangeUserClicked()
{
	ensure(ShouldDisplayChangeUserButton());

	UE_LOG(LogHunter, Log, TEXT("[%hs] Change user requested!"), __func__);
...
void UHunterControllerDisconnectedScreen::HandleChangeUserCompleted(const FPlatformUserSelectionCompleteParams& Params)
{
	UE_LOG(LogHunter, Log, TEXT("[%hs] User change complete!"), __func__);
