...
#include "UI/Foundation/HunterControllerDisconnectedScreen.h"
#include "UI/HunterActivatableWidget.h"

#if WITH_EDITOR
#include "CommonUIVisibilitySubsystem.h"
#endif	// WITH_EDITOR

#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterHUDLayout)
...
UHunterHUDLayout::UHunterHUDLayout(const FObjectInitializer& ObjectInitializer)
...
void UHunterHUDLayout::NativeOnInitialized()
{
...
void UHunterHUDLayout::NativeDestruct()
{
...
void UHunterHUDLayout::HandleEscapeAction()
{
...
void UHunterHUDLayout::HandleInputDeviceConnectionChanged(EInputDeviceConnectionState NewConnectionState, FPlatformUserId PlatformUserId, FInputDeviceId InputDeviceId)
{
...
void UHunterHUDLayout::HandleInputDevicePairingChanged(FInputDeviceId InputDeviceId, FPlatformUserId NewUserPlatformId, FPlatformUserId OldUserPlatformId)
{
	const FPlatformUserId OwningLocalPlayerId = GetOwningLocalPlayer()->GetPlatformUserId();
...
bool UHunterHUDLayout::ShouldPlatformDisplayControllerDisconnectScreen() const
{
...
void UHunterHUDLayout::NotifyControllerStateChangeForDisconnectScreen()
{
...
void UHunterHUDLayout::ProcessControllerDevicesHavingChangedForDisconnectScreen()
{
	// We should only ever get here if we have bound to the controller state change delegates
	ensure(ShouldPlatformDisplayControllerDisconnectScreen());
...
void UHunterHUDLayout::DisplayControllerDisconnectedMenu_Implementation()
{
	UE_LOG(LogHunter, Log, TEXT("[%hs] Display controller disconnected menu!"), __func__);
...
void UHunterHUDLayout::HideControllerDisconnectedMenu_Implementation()
{
	UE_LOG(LogHunter, Log, TEXT("[%hs] Hide controller disconnected menu!"), __func__);
