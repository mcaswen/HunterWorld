...
#include "HunterFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterFrontendStateComponent)
...
UHunterFrontendStateComponent::UHunterFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UHunterFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UHunterFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UHunterFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UHunterFrontendStateComponent::OnExperienceLoaded(const ULyraExperienceDefinition* Experience)
{
...
void UHunterFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UHunterFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UHunterFrontendStateComponent::OnUserInitialized);
...
void UHunterFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UHunterFrontendStateComponent::OnUserInitialized);
...
void UHunterFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UHunterFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
