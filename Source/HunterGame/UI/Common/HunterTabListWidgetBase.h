...
#include "HunterTabListWidgetBase.generated.h"
...
struct FHunterTabDescriptor
{
	GENERATED_BODY()

public:
	FHunterTabDescriptor()
	: bHidden(false)
...
class UHunterTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class IHunterTabButtonInterface
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabelInfo(const FHunterTabDescriptor& TabDescriptor);
};

UCLASS(MinimalAPI, Blueprintable, BlueprintType, Abstract, meta = (DisableNativeTick))
class UHunterTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
	
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
	UE_API bool GetPreregisteredTabInfo(const FName TabNameId, FHunterTabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FHunterTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	UE_API bool RegisterDynamicTab(const FHunterTabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(UHunterTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
private:
	...
	TArray<FHunterTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FHunterTabDescriptor> PendingTabLabelInfoMap;
};
