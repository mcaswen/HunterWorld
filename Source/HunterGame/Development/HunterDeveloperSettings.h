...
#include "HunterDeveloperSettings.generated.h"
...
class UHunterDeveloperSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UHunterDeveloperSettings();
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=Hunter, meta=(AllowedTypes="LyraExperienceDefinition"))
	FPrimaryAssetId ExperienceOverride;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=HunterBots, meta=(InlineEditConditionToggle))
	bool bOverrideBotCount = false;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=HunterBots, meta=(EditCondition=bOverrideBotCount))
	int32 OverrideNumPlayerBotsToSpawn = 0;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=HunterBots)
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=Hunter)
	bool bTestFullGameFlowInPIE = false;

	/**
	* Should force feedback effects be played, even if the last input device was not a gamepad?
	* The default behavior in Hunter is to only play force feedback if the most recent input device was a gamepad.
	*/
	UPROPERTY(config, EditAnywhere, Category = Hunter, meta = (ConsoleVariable = "HunterPC.ShouldAlwaysPlayForceFeedback"))
	...
	UPROPERTY(config, EditAnywhere, Category=Hunter)
	TArray<FHunterCheatToRun> CheatsToRun;
