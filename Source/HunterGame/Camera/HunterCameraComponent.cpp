...
UHunterCameraComponent::UHunterCameraComponent(const FObjectInitializer& ObjectInitializer)
...
void UHunterCameraComponent::OnRegister()
{
	Super::OnRegister();
...
void UHunterCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& DesiredView)
{
	check(CameraModeStack);
...
void UHunterCameraComponent::UpdateCameraModes()
{
	check(CameraModeStack);
...
void UHunterCameraComponent::DrawDebug(UCanvas* Canvas) const
{
	check(Canvas);
	...
	DisplayDebugManager.DrawString(FString::Printf(TEXT("HunterCameraComponent: %s"), *GetNameSafe(GetTargetActor())));
...
void UHunterCameraComponent::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	check(CameraModeStack);
