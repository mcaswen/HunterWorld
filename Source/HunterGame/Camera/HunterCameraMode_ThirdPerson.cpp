...
#include "HunterCameraMode_ThirdPerson.h"
#include "Camera/HunterCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/HunterPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "HunterCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterCameraMode_ThirdPerson)

namespace HunterCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UHunterCameraMode_ThirdPerson::UHunterCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FHunterPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FHunterPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHunterPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHunterPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHunterPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHunterPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FHunterPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UHunterCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UHunterCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UHunterCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UHunterCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IHunterCameraAssistInterface* TargetControllerAssist = Cast<IHunterCameraAssistInterface>(TargetController);

	IHunterCameraAssistInterface* TargetActorAssist = Cast<IHunterCameraAssistInterface>(TargetActor);
	...
	IHunterCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IHunterCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IHunterCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IHunterCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UHunterCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IHunterCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FHunterPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(HunterCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UHunterCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UHunterCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
