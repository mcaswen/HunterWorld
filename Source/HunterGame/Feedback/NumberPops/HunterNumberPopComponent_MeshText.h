...
#include "HunterNumberPopComponent_MeshText.generated.h"

class UHunterDamagePopStyle;
...
class UHunterNumberPopComponent_MeshText : public UHunterNumberPopComponent
{
	GENERATED_BODY()

public:

	UHunterNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FHunterNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo, const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FHunterNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FHunterNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<UHunterDamagePopStyle>> Styles;
