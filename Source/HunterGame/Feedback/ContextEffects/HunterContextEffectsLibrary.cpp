...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterContextEffectsLibrary)


void UHunterContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UHunterContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UHunterContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UHunterContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FHunterContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UHunterActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FHunterContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UHunterActiveContextEffects* NewActiveContextEffects = NewObject<UHunterActiveContextEffects>(this);
	...
	this->HunterContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UHunterContextEffectsLibrary::HunterContextEffectLibraryLoadingComplete(
	TArray<UHunterActiveContextEffects*> HunterActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(HunterActiveContextEffects);
}
