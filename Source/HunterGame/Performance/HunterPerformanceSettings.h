...
#include "HunterPerformanceSettings.generated.h"
...
USTRUCT()
struct FHunterQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FHunterPerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<EHunterDisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class EHunterFramePacingMode : uint8
{
...
class UHunterPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UHunterPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UHunterPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FHunterQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EHunterFramePacingMode FramePacingMode = EHunterFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by Hunter.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EHunterFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Hunter Performance Settings"))
class UHunterPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UHunterPerformanceSettings();
...
public:
	...
	TArray<FHunterPerformanceStatGroup> UserFacingPerformanceStats;
};
