...
#include "HunterPerformanceStatTypes.h"
#include "Algo/MaxElement.h"
#include "Algo/MinElement.h"
#include "Stats/StatsData.h"
#include "Subsystems/GameInstanceSubsystem.h"

#include "HunterPerformanceStatSubsystem.generated.h"

enum class EHunterDisplayablePerformanceStat : uint8;

class FSubsystemCollectionBase;
class UHunterPerformanceStatSubsystem;
class UObject;
struct FFrame;
...
struct FHunterPerformanceStatCache : public IPerformanceDataConsumer
{
public:
	FHunterPerformanceStatCache(UHunterPerformanceStatSubsystem* InSubsystem)
	...
	*/
	double GetCachedStat(EHunterDisplayablePerformanceStat Stat) const;
	 ...
	 */
	const FSampledStatCache* GetCachedStatData(const EHunterDisplayablePerformanceStat Stat) const;

protected:

	void RecordStat(const EHunterDisplayablePerformanceStat Stat, const double Value);
	
	UHunterPerformanceStatSubsystem* MySubsystem;

	/**
	 * Caches the sampled data for each of the performance stats currently available
	 */
	TMap<EHunterDisplayablePerformanceStat, FSampledStatCache> PerfStateCache;
};
...
class UHunterPerformanceStatSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	TSharedPtr<FHunterPerformanceStatCache> Tracker;
};
