...
#include "HunterBotCreationComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterBotCreationComponent)

UHunterBotCreationComponent::UHunterBotCreationComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UHunterBotCreationComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	ExperienceComponent->CallOrRegister_OnExperienceLoaded_LowPriority(FOnHunterExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::OnExperienceLoaded));
}

void UHunterBotCreationComponent::OnExperienceLoaded(const ULyraExperienceDefinition* Experience)
{
#if WITH_SERVER_CODE
	if (HasAuthority())
	{
...
void UHunterBotCreationComponent::ServerCreateBots_Implementation()
{
	...
	if (GIsEditor)
	{
		const UHunterDeveloperSettings* DeveloperSettings = GetDefault<UHunterDeveloperSettings>();
...
FString UHunterBotCreationComponent::CreateBotName(int32 PlayerIndex)
{
	FString Result;
...
void UHunterBotCreationComponent::SpawnOneBot()
{
	...
	{
		AHunterGameMode* GameMode = GetGameMode<AHunterGameMode>();
		check(GameMode);
		...
		{
			if (UHunterPawnExtensionComponent* PawnExtComponent = NewController->GetPawn()->FindComponentByClass<UHunterPawnExtensionComponent>())
			{
...
void UHunterBotCreationComponent::RemoveOneBot()
{
	if (SpawnedBotList.Num() > 0)
	{
		...
		if (BotToRemove)
		{
			...
			{
				if (UHunterHealthComponent* HealthComponent = UHunterHealthComponent::FindHealthComponent(ControlledPawn))
				{
...
void UHunterBotCreationComponent::ServerCreateBots_Implementation()
{
	ensureMsgf(0, TEXT("Bot functions do not exist in HunterClient!"));
}
