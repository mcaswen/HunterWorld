...
namespace Hunter
{
	...
	static FAutoConsoleVariableRef CVarHunterTestEncryption(
		TEXT("Hunter.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarHunterUseDTLSEncryption(
		TEXT("Hunter.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using Hunter.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarHunterTestDTLSFingerprint(
		TEXT("Hunter.TestDTLSFingerprint"),
...
void ULyraGameInstance::Init()
{
	...
	{
		ComponentManager->RegisterInitState(HunterGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(HunterGameplayTags::InitState_DataAvailable, false, HunterGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(HunterGameplayTags::InitState_DataInitialized, false, HunterGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(HunterGameplayTags::InitState_GameplayReady, false, HunterGameplayTags::InitState_DataInitialized);
	}
...
AHunterPlayerController* ULyraGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AHunterPlayerController>(Super::GetPrimaryPlayerController(false));
}
...
void ULyraGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UHunterLocalPlayer* LocalPlayer = Cast<UHunterLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void ULyraGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Hunter::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Hunter::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("HunterTest.pem");
