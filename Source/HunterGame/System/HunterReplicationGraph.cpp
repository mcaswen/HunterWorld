...
/**
*	
*	===================== HunterReplicationGraph Replication =====================
*
*	Overview
*	
*		This changes the way actor relevancy works. AActor::IsNetRelevantFor is NOT used in this system!
*		
*		Instead, The UHunterReplicationGraph contains UReplicationGraphNodes. These nodes are responsible for generating lists of actors to replicate for each connection.
...
*		
*		UHunterReplicationGraph is intended to not be directly used by the game code. That is, you should not have to include HunterReplicationGraph.h anywhere else.
*		Rather, UHunterReplicationGraph depends on the game code and registers for events that the game code broadcasts (e.g., events for players joining/leaving teams).
*		This choice was made because it gives UHunterReplicationGraph a complete holistic view of actor replication. Rather than exposing generic public functions that any
*		place in game code can invoke, all notifications are explicitly registered in UHunterReplicationGraph::InitGlobalActorClassSettings.
*		
*	Hunter Nodes
*	
...
*		
*		UHunterReplicationGraphNode_AlwaysRelevant_ForConnection
...
*		
*		UHunterReplicationGraphNode_PlayerStateFrequencyLimiter
...
*		owning connection only) via UHunterReplicationGraphNode_AlwaysRelevant_ForConnection.
*		
...
*		
*		Making something always relevant to connection: You will need to modify UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection. You will also want 
*		to make sure the actor does not get put in one of the other nodes. The safest way to do this is by setting its EClassRepNodeMapping to NotRouted in UHunterReplicationGraph::InitGlobalActorClassSettings.
*
*	How To Debug
*	
...
*		
*		Hunter.RepGraph.PrintRouting - will print the EClassRepNodeMapping for each class. That is, how a given actor class is routed (or not) in the Replication Graph.
*	
*/

#include "HunterReplicationGraph.h"
...
DEFINE_LOG_CATEGORY( LogHunterRepGraph );

namespace Hunter::RepGraph
{
	float DestructionInfoMaxDist = 30000.f;
	static FAutoConsoleVariableRef CVarHunterRepGraphDestructMaxDist(TEXT("Hunter.RepGraph.DestructInfo.MaxDist"), DestructionInfoMaxDist, TEXT("Max distance (not squared) to rep destruct infos at"), ECVF_Default);

	int32 DisplayClientLevelStreaming = 0;
	static FAutoConsoleVariableRef CVarHunterRepGraphDisplayClientLevelStreaming(TEXT("Hunter.RepGraph.DisplayClientLevelStreaming"), DisplayClientLevelStreaming, TEXT(""), ECVF_Default);

	float CellSize = 10000.f;
	static FAutoConsoleVariableRef CVarHunterRepGraphCellSize(TEXT("Hunter.RepGraph.CellSize"), CellSize, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarHunterRepGraphSpatialBiasX(TEXT("Hunter.RepGraph.SpatialBiasX"), SpatialBiasX, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarHunterRepSpatialBiasY(TEXT("Hunter.RepGraph.SpatialBiasY"), SpatialBiasY, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarHunterRepDynamicActorFrequencyBuckets(TEXT("Hunter.RepGraph.DynamicActorFrequencyBuckets"), DynamicActorFrequencyBuckets, TEXT(""), ECVF_Default);

	int32 DisableSpatialRebuilds = 1;
	static FAutoConsoleVariableRef CVarHunterRepDisableSpatialRebuilds(TEXT("Hunter.RepGraph.DisableSpatialRebuilds"), DisableSpatialRebuilds, TEXT(""), ECVF_Default);

	int32 LogLazyInitClasses = 0;
	static FAutoConsoleVariableRef CVarHunterRepLogLazyInitClasses(TEXT("Hunter.RepGraph.LogLazyInitClasses"), LogLazyInitClasses, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarHunterRepTargetKBytesSecFastSharedPath(TEXT("Hunter.RepGraph.TargetKBytesSecFastSharedPath"), TargetKBytesSecFastSharedPath, TEXT(""), ECVF_Default);

	float FastSharedPathCullDistPct = 0.80f;
	static FAutoConsoleVariableRef CVarHunterRepFastSharedPathCullDistPct(TEXT("Hunter.RepGraph.FastSharedPathCullDistPct"), FastSharedPathCullDistPct, TEXT(""), ECVF_Default);

	int32 EnableFastSharedPath = 1;
	static FAutoConsoleVariableRef CVarHunterRepEnableFastSharedPath(TEXT("Hunter.RepGraph.EnableFastSharedPath"), EnableFastSharedPath, TEXT(""), ECVF_Default);
	...
	UReplicationDriver* ConditionalCreateReplicationDriver(UNetDriver* ForNetDriver, UWorld* World)
	{
		...
		{
			const UHunterReplicationGraphSettings* HunterRepGraphSettings = GetDefault<UHunterReplicationGraphSettings>();

			// Enable/Disable via developer settings
			if (HunterRepGraphSettings && HunterRepGraphSettings->bDisableReplicationGraph)
			{
				UE_LOG(LogHunterRepGraph, Display, TEXT("Replication graph is disabled via HunterReplicationGraphSettings."));
				return nullptr;
			}

			UE_LOG(LogHunterRepGraph, Display, TEXT("Replication graph is enabled for %s in world %s."), *GetNameSafe(ForNetDriver), *GetPathNameSafe(World));

			TSubclassOf<UHunterReplicationGraph> GraphClass = HunterRepGraphSettings->DefaultReplicationGraphClass.TryLoadClass<UHunterReplicationGraph>();
			if (GraphClass.Get() == nullptr)
			{
				GraphClass = UHunterReplicationGraph::StaticClass();
			}

			UHunterReplicationGraph* HunterReplicationGraph = NewObject<UHunterReplicationGraph>(GetTransientPackage(), GraphClass.Get());
			return HunterReplicationGraph;
		}
...
UHunterReplicationGraph::UHunterReplicationGraph()
{
	if (!UReplicationDriver::CreateReplicationDriverDelegate().IsBound())
	{
		UReplicationDriver::CreateReplicationDriverDelegate().BindLambda(
			...
			{
				return Hunter::RepGraph::ConditionalCreateReplicationDriver(ForNetDriver, World);
			});
	}
}

void UHunterReplicationGraph::ResetGameWorldState()
{
	...
	for (UNetReplicationGraphConnection* ConnManager : Connections)
	{
		...
		{
			if (UHunterReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = Cast<UHunterReplicationGraphNode_AlwaysRelevant_ForConnection>(ConnectionNode))
			{
...
EClassRepNodeMapping UHunterReplicationGraph::GetClassNodeMapping(UClass* Class) const
{
	if (!Class)
	{
...
void UHunterReplicationGraph::RegisterClassRepNodeMapping(UClass* Class)
{
	EClassRepNodeMapping Mapping = GetClassNodeMapping(Class);
	ClassRepNodePolicies.Set(Class, Mapping);
}

void UHunterReplicationGraph::InitClassReplicationInfo(FClassReplicationInfo& Info, UClass* Class, bool Spatialize) const
{
	AActor* CDO = Class->GetDefaultObject<AActor>();
	if (Spatialize)
	{
		Info.SetCullDistanceSquared(CDO->GetNetCullDistanceSquared());
		UE_LOG(LogHunterRepGraph, Log, TEXT("Setting cull distance for %s to %f (%f)"), *Class->GetName(), Info.GetCullDistanceSquared(), Info.GetCullDistance());
	}
	...
	UE_LOG(LogHunterRepGraph, Log, TEXT("Setting replication period for %s (%s) to %d frames (%.2f)"), *Class->GetName(), *NativeClass->GetName(), Info.ReplicationPeriodFrame, CDO->GetNetUpdateFrequency());
}

bool UHunterReplicationGraph::ConditionalInitClassReplicationInfo(UClass* ReplicatedClass, FClassReplicationInfo& ClassInfo)
{
...
void UHunterReplicationGraph::AddClassRepInfo(UClass* Class, EClassRepNodeMapping Mapping)
{
	if (IsSpatialized(Mapping))
	{
		if (Class->GetDefaultObject<AActor>()->bAlwaysRelevant)
		{
			UE_LOG(LogHunterRepGraph, Warning, TEXT("Replicated Class %s is AlwaysRelevant but is initialized into a spatialized node (%s)"), *Class->GetName(), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
		}
	}
...
void UHunterReplicationGraph::RegisterClassReplicationInfo(UClass* ReplicatedClass)
{
	...
	if (ConditionalInitClassReplicationInfo(ReplicatedClass, ClassInfo))
	{
		...
		UE_LOG(LogHunterRepGraph, Log, TEXT("Setting %s - %.2f"), *GetNameSafe(ReplicatedClass), ClassInfo.GetCullDistance());
	}
}

void UHunterReplicationGraph::InitGlobalActorClassSettings()
{
	...
	GlobalActorReplicationInfoMap.SetInitClassInfoFunc(
		[this](UClass* Class, FClassReplicationInfo& ClassInfo)
		{
			...
			if (Hunter::RepGraph::LogLazyInitClasses != 0)
			{
				if (bHandled)
				{
					EClassRepNodeMapping Mapping = ClassRepNodePolicies.GetChecked(Class);
					UE_LOG(LogHunterRepGraph, Warning, TEXT("%s was Lazy Initialized. (Parent: %s) %d."), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()), (int32)Mapping);
					...
					{
						UE_LOG(LogHunterRepGraph, Warning, TEXT("Differences Found!"));
						FString DebugStr = ParentRepInfo.BuildDebugStringDelta();
						UE_LOG(LogHunterRepGraph, Warning, TEXT("  Parent: %s"), *DebugStr);

						DebugStr = ClassInfo.BuildDebugStringDelta();
						UE_LOG(LogHunterRepGraph, Warning, TEXT("  Class : %s"), *DebugStr);
					}
				}
				else
				{
					UE_LOG(LogHunterRepGraph, Warning, TEXT("%s skipped Lazy Initialization because it does not differ from its parent. (Parent: %s)"), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()));
	...
	const UHunterReplicationGraphSettings* HunterRepGraphSettings = GetDefault<UHunterReplicationGraphSettings>();
	check(HunterRepGraphSettings);

	// Set Classes Node Mappings
	for (const FRepGraphActorClassSettings& ActorClassSettings : HunterRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddClassRepInfoToMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(LogHunterRepGraph, Log, TEXT("ActorClassSettings -- AddClassRepInfo - %s :: %i"), *StaticActorClass->GetName(), int(ActorClassSettings.ClassNodeMapping));
...
#if WITH_GAMEPLAY_DEBUGGER
	AddClassRepInfo(AGameplayDebuggerCategoryReplicator::StaticClass(), EClassRepNodeMapping::NotRouted);				// Replicated via UHunterReplicationGraphNode_AlwaysRelevant_ForConnection
#endif
	...
	CharacterClassRepInfo.SetCullDistanceSquared(AHunterCharacter::StaticClass()->GetDefaultObject<AHunterCharacter>()->GetNetCullDistanceSquared());
...

	{
		...
		FRepMovement DefaultRepMovement = AHunterCharacter::StaticClass()->GetDefaultObject<AHunterCharacter>()->GetReplicatedMovement(); // Use the same quantization settings as our default replicatedmovement
	...
	{
		bool bSuccess = false;
		if (AHunterCharacter* Character = Cast<AHunterCharacter>(Actor))
		{
	...
	FastSharedPathConstants.MaxBitsPerFrame = (int32)((float)(Hunter::RepGraph::TargetKBytesSecFastSharedPath * 1024 * 8) / NetDriver->GetNetServerMaxTickRate());
	FastSharedPathConstants.DistanceRequirementPct = Hunter::RepGraph::FastSharedPathCullDistPct;

	SetClassInfo(AHunterCharacter::StaticClass(), CharacterClassRepInfo);

	// ---------------------------------------------------------------------
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.ListSize = 12;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.NumBuckets = Hunter::RepGraph::DynamicActorFrequencyBuckets;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.BucketThresholds.Reset();
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.EnableFastPath = (Hunter::RepGraph::EnableFastSharedPath > 0);
	...
	UE_LOG(LogHunterRepGraph, Log, TEXT(""));
	UE_LOG(LogHunterRepGraph, Log, TEXT("Class Routing Map: "));
	for (auto ClassMapIt = ClassRepNodePolicies.CreateIterator(); ClassMapIt; ++ClassMapIt)
	{
		...
		UE_LOG(LogHunterRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(ParentNativeClass), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
	}

	UE_LOG(LogHunterRepGraph, Log, TEXT(""));
	UE_LOG(LogHunterRepGraph, Log, TEXT("Class Settings Map: "));
	...
	for (auto ClassRepInfoIt = GlobalActorReplicationInfoMap.CreateClassMapIterator(); ClassRepInfoIt; ++ClassRepInfoIt)
	{
		...
		UE_LOG(LogHunterRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(GetParentNativeClass(Class)), *ClassInfo.BuildDebugStringDelta());
	}


	// Rep destruct infos based on CVar value
	DestructInfoMaxDistanceSquared = Hunter::RepGraph::DestructionInfoMaxDist * Hunter::RepGraph::DestructionInfoMaxDist;
	...
	for (const FRepGraphActorClassSettings& ActorClassSettings : HunterRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddToRPC_Multicast_OpenChannelForClassMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(LogHunterRepGraph, Log, TEXT("ActorClassSettings -- RPC_Multicast_OpenChannelForClass - %s"), *StaticActorClass->GetName());
...
void UHunterReplicationGraph::InitGlobalGraphNodes()
{
	// -----------------------------------------------
	//	Spatial Actors
	// -----------------------------------------------
	...
	GridNode->CellSize = Hunter::RepGraph::CellSize;
	GridNode->SpatialBias = FVector2D(Hunter::RepGraph::SpatialBiasX, Hunter::RepGraph::SpatialBiasY);

	if (Hunter::RepGraph::DisableSpatialRebuilds)
	{
	...
	// -----------------------------------------------
	UHunterReplicationGraphNode_PlayerStateFrequencyLimiter* PlayerStateNode = CreateNewNode<UHunterReplicationGraphNode_PlayerStateFrequencyLimiter>();
	AddGlobalGraphNode(PlayerStateNode);
}

void UHunterReplicationGraph::InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
	Super::InitConnectionGraphNodes(RepGraphConnection);

	UHunterReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = CreateNewNode<UHunterReplicationGraphNode_AlwaysRelevant_ForConnection>();

	// This node needs to know when client levels go in and out of visibility
	RepGraphConnection->OnClientVisibleLevelNameAdd.AddUObject(AlwaysRelevantConnectionNode, &UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd);
	RepGraphConnection->OnClientVisibleLevelNameRemove.AddUObject(AlwaysRelevantConnectionNode, &UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove);

	AddConnectionGraphNode(AlwaysRelevantConnectionNode, RepGraphConnection);
}

EClassRepNodeMapping UHunterReplicationGraph::GetMappingPolicy(UClass* Class)
{
...
void UHunterReplicationGraph::RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
...
void UHunterReplicationGraph::RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo)
{
	EClassRepNodeMapping Policy = GetMappingPolicy(ActorInfo.Class);
	switch(Policy)
	{
		...
		case EClassRepNodeMapping::RelevantAllConnections:
		{
			...
			else
			{
				...
				{
					UE_LOG(LogHunterRepGraph, Warning, TEXT("Actor %s was not found in AlwaysRelevantStreamingLevelActors list. LevelName: %s"), *GetActorRepListTypeDebugString(ActorInfo.Actor), *ActorInfo.StreamingLevelName.ToString());
				}				
			}
...
#if WITH_GAMEPLAY_DEBUGGER
void UHunterReplicationGraph::OnGameplayDebuggerOwnerChange(AGameplayDebuggerCategoryReplicator* Debugger, APlayerController* OldOwner)
{
	CHECK_WORLDS(Debugger);

	auto GetAlwaysRelevantForConnectionNode = [this](APlayerController* Controller) -> UHunterReplicationGraphNode_AlwaysRelevant_ForConnection*
	{
		if (Controller)
		{
	...
	if (UHunterReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(OldOwner))
	{
		AlwaysRelevantConnectionNode->GameplayDebugger = nullptr;
	}

	if (UHunterReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(Debugger->GetReplicationOwner()))
	{
...
void UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::ResetGameWorldState()
{
	ReplicationActorList.Reset();
	AlwaysRelevantStreamingLevelsNeedingReplication.Empty();
}

void UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	UHunterReplicationGraph* HunterGraph = CastChecked<UHunterReplicationGraph>(GetOuter());
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (AHunterPlayerController* PC = Cast<AHunterPlayerController>(CurViewer.InViewer))
		{
			...
			if (bReplicatePS)
			{
				// Always return the player state to the owning player. Simulated proxy player states are handled by UHunterReplicationGraphNode_PlayerStateFrequencyLimiter
			...
			if (AHunterCharacter* Pawn = Cast<AHunterCharacter>(PC->GetPawn()))
			{
			...
			if (AHunterCharacter* ViewTargetPawn = Cast<AHunterCharacter>(CurViewer.ViewTarget))
			{
	...
	TMap<FName, FActorRepListRefView>& AlwaysRelevantStreamingLevelActors = HunterGraph->AlwaysRelevantStreamingLevelActors;
	...
	for (int32 Idx=AlwaysRelevantStreamingLevelsNeedingReplication.Num()-1; Idx >= 0; --Idx)
	{
		...
		if (Ptr == nullptr)
		{
			...
			UE_CLOG(Hunter::RepGraph::DisplayClientLevelStreaming > 0, LogHunterRepGraph, Display, TEXT("CLIENTSTREAMING Removing %s from AlwaysRelevantStreamingLevelActors because FActorRepListRefView is null. %s "), *StreamingLevel.ToString(),  *Params.ConnectionManager.GetName());
		...
		if (RepList.Num() > 0)
		{
			...
			if (bAllDormant)
			{
				UE_CLOG(Hunter::RepGraph::DisplayClientLevelStreaming > 0, LogHunterRepGraph, Display, TEXT("CLIENTSTREAMING All AlwaysRelevant Actors Dormant on StreamingLevel %s for %s. Removing list."), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				AlwaysRelevantStreamingLevelsNeedingReplication.RemoveAtSwap(Idx, EAllowShrinking::No);
			}
			else
			{
				UE_CLOG(Hunter::RepGraph::DisplayClientLevelStreaming > 0, LogHunterRepGraph, Display, TEXT("CLIENTSTREAMING Adding always Actors on StreamingLevel %s for %s because it has at least one non dormant actor"), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				Params.OutGatheredReplicationLists.AddReplicationActorList(RepList);
			}
		}
		else
		{
			UE_LOG(LogHunterRepGraph, Warning, TEXT("UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection - empty RepList %s"), *Params.ConnectionManager.GetName());
		}
...
void UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd(FName LevelName, UWorld* StreamingWorld)
{
	UE_CLOG(Hunter::RepGraph::DisplayClientLevelStreaming > 0, LogHunterRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityAdd - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Add(LevelName);
}

void UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove(FName LevelName)
{
	UE_CLOG(Hunter::RepGraph::DisplayClientLevelStreaming > 0, LogHunterRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityRemove - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Remove(LevelName);
}

void UHunterReplicationGraphNode_AlwaysRelevant_ForConnection::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	...
	{
		UHunterReplicationGraph* HunterGraph = CastChecked<UHunterReplicationGraph>(GetOuter());
		if (FActorRepListRefView* RepList = HunterGraph->AlwaysRelevantStreamingLevelActors.Find(LevelName))
		{
...
UHunterReplicationGraphNode_PlayerStateFrequencyLimiter::UHunterReplicationGraphNode_PlayerStateFrequencyLimiter()
{
	bRequiresPrepareForReplicationCall = true;
}

void UHunterReplicationGraphNode_PlayerStateFrequencyLimiter::PrepareForReplication()
{
...
void UHunterReplicationGraphNode_PlayerStateFrequencyLimiter::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
...
void UHunterReplicationGraphNode_PlayerStateFrequencyLimiter::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	DebugInfo.Log(NodeName);
	DebugInfo.PushIndent();	
...
void UHunterReplicationGraph::PrintRepNodePolicies()
{
	...
	GLog->Logf(TEXT("===================================="));
	GLog->Logf(TEXT("Hunter Replication Routing Policies"));
	GLog->Logf(TEXT("===================================="));
...
FAutoConsoleCommandWithWorldAndArgs HunterPrintRepNodePoliciesCmd(TEXT("Hunter.RepGraph.PrintRouting"),TEXT("Prints how actor classes are routed to RepGraph nodes"),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		for (TObjectIterator<UHunterReplicationGraph> It; It; ++It)
		{
...
FAutoConsoleCommandWithWorldAndArgs ChangeFrequencyBucketsCmd(TEXT("Hunter.RepGraph.FrequencyBuckets"), TEXT("Resets frequency bucket count."), FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray< FString >& Args, UWorld* World) 
{
	int32 Buckets = 1;
	if (Args.Num() > 0)
	{
	...
	UE_LOG(LogHunterRepGraph, Display, TEXT("Setting Frequency Buckets to %d"), Buckets);
