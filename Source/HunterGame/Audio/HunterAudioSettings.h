...
#include "HunterAudioSettings.generated.h"
...
USTRUCT()
struct FHunterSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(MinimalAPI, config = Game, defaultconfig, meta = (DisplayName = "HunterAudioSettings"))
class UHunterAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FHunterSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FHunterSubmixEffectChainMap> LDRAudioSubmixEffectChain;
