...
#include "HunterDamageExecution.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UHunterCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UHunterDamageExecution::UHunterDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UHunterDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FHunterGameplayEffectContext* TypedContext = FHunterGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UHunterTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UHunterTeamSubsystem>();
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UHunterHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
