...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			{
				UHunterGameplayAbility* HunterAbilityInstance = Cast<UHunterGameplayAbility>(AbilityInstance);
...
void UHunterAbilitySystemComponent::TryActivateAbilitiesOnSpawn()
{
	...
	for (const FGameplayAbilitySpec& AbilitySpec : ActivatableAbilities.Items)
	{
		if (const UHunterGameplayAbility* HunterAbilityCDO = Cast<UHunterGameplayAbility>(AbilitySpec.Ability))
		{
...
void UHunterAbilitySystemComponent::CancelAbilitiesByFunc(TShouldCancelAbilityFunc ShouldCancelFunc, bool bReplicateCancelAbility)
{
	...
	for (const FGameplayAbilitySpec& AbilitySpec : ActivatableAbilities.Items)
	{
		...
		UHunterGameplayAbility* HunterAbilityCDO = Cast<UHunterGameplayAbility>(AbilitySpec.Ability);
		if (!HunterAbilityCDO)
		{
			UE_LOG(LogHunterAbilitySystem, Error, TEXT("CancelAbilitiesByFunc: Non-HunterGameplayAbility %s was Granted to ASC. Skipping."), *AbilitySpec.Ability.GetName());
			continue;
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		for (UGameplayAbility* AbilityInstance : Instances)
		{
			UHunterGameplayAbility* HunterAbilityInstance = CastChecked<UHunterGameplayAbility>(AbilityInstance);
...
{
	auto ShouldCancelFunc = [this](const UHunterGameplayAbility* HunterAbility, FGameplayAbilitySpecHandle Handle)
	{
...
void UHunterAbilitySystemComponent::ProcessAbilityInput(float DeltaTime, bool bGamePaused)
{
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputHeldSpecHandles)
	{
		if (const FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			...
			{
				const UHunterGameplayAbility* HunterAbilityCDO = Cast<UHunterGameplayAbility>(AbilitySpec->Ability);
...
void UHunterAbilitySystemComponent::NotifyAbilityActivated(const FGameplayAbilitySpecHandle Handle, UGameplayAbility* Ability)
{
	...
	if (UHunterGameplayAbility* HunterAbility = Cast<UHunterGameplayAbility>(Ability))
	{
...
void UHunterAbilitySystemComponent::HandleAbilityFailed(const UGameplayAbility* Ability, const FGameplayTagContainer& FailureReason)
{
	...
	if (const UHunterGameplayAbility* HunterAbility = Cast<const UHunterGameplayAbility>(Ability))
	{
...
void UHunterAbilitySystemComponent::AddAbilityToActivationGroup(EHunterAbilityActivationGroup Group, UHunterGameplayAbility* HunterAbility)
{
	check(HunterAbility);
...
void UHunterAbilitySystemComponent::RemoveAbilityFromActivationGroup(EHunterAbilityActivationGroup Group, UHunterGameplayAbility* HunterAbility)
{
	check(HunterAbility);
...
void UHunterAbilitySystemComponent::CancelActivationGroupAbilities(EHunterAbilityActivationGroup Group, UHunterGameplayAbility* IgnoreHunterAbility, bool bReplicateCancelAbility)
{
	auto ShouldCancelFunc = [this, Group, IgnoreHunterAbility](const UHunterGameplayAbility* HunterAbility, FGameplayAbilitySpecHandle Handle)
	{
