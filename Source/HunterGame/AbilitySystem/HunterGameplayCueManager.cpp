...
#include "HunterGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EHunterEditorLoadMode
{
...
namespace HunterGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Hunter.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via HunterGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UHunterGameplayCueManager::DumpGameplayCues));

	static EHunterEditorLoadMode LoadMode = EHunterEditorLoadMode::LoadUpfront;
}
...
UHunterGameplayCueManager::UHunterGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UHunterGameplayCueManager* UHunterGameplayCueManager::Get()
{
	return Cast<UHunterGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UHunterGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UHunterGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogHunter, Warning, TEXT("UHunterGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UHunterGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (HunterGameplayCueManagerCvars::LoadMode)
	{
	case EHunterEditorLoadMode::LoadUpfront:
		return true;
	case EHunterEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EHunterEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UHunterGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UHunterGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UHunterGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UHunterGameplayCueManager* GCM = Cast<UHunterGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogHunter, Error, TEXT("DumpGameplayCues failed. No UHunterGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogHunter, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogHunter, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogHunter, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogHunter, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogHunter, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogHunter, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogHunter, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogHunter, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogHunter, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogHunter, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogHunter, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogHunter, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UHunterGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UHunterGameplayCueManager* StrongThis = Get())
					{
...
void UHunterGameplayCueManager::HandlePostGarbageCollect()
{
...
void UHunterGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogHunter, Warning, TEXT("UHunterGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UHunterGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (HunterGameplayCueManagerCvars::LoadMode)
	{
	case EHunterEditorLoadMode::LoadUpfront:
...
void UHunterGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UHunterGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UHunterGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UHunterGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UHunterGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UHunterGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
