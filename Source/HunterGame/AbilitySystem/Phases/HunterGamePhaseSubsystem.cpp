...
UHunterGamePhaseSubsystem::UHunterGamePhaseSubsystem()
{
}

void UHunterGamePhaseSubsystem::PostInitialize()
{
	Super::PostInitialize();
}

bool UHunterGamePhaseSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
bool UHunterGamePhaseSubsystem::DoesSupportWorldType(const EWorldType::Type WorldType) const
{
...
void UHunterGamePhaseSubsystem::StartPhase(TSubclassOf<UHunterGamePhaseAbility> PhaseAbility, FHunterGamePhaseDelegate PhaseEndedCallback)
{
	...
	if (ensure(GameState_ASC))
	{
		...
		{
			FHunterGamePhaseEntry& Entry = ActivePhaseMap.FindOrAdd(SpecHandle);
...
void UHunterGamePhaseSubsystem::K2_StartPhase(TSubclassOf<UHunterGamePhaseAbility> PhaseAbility, const FHunterGamePhaseDynamicDelegate& PhaseEndedDelegate)
{
	const FHunterGamePhaseDelegate EndedDelegate = FHunterGamePhaseDelegate::CreateWeakLambda(const_cast<UObject*>(PhaseEndedDelegate.GetUObject()), [PhaseEndedDelegate](const UHunterGamePhaseAbility* PhaseAbility) {
...
void UHunterGamePhaseSubsystem::K2_WhenPhaseStartsOrIsActive(FGameplayTag PhaseTag, EPhaseTagMatchType MatchType, FHunterGamePhaseTagDynamicDelegate WhenPhaseActive)
{
	const FHunterGamePhaseTagDelegate ActiveDelegate = FHunterGamePhaseTagDelegate::CreateWeakLambda(WhenPhaseActive.GetUObject(), [WhenPhaseActive](const FGameplayTag& PhaseTag) {
...
void UHunterGamePhaseSubsystem::K2_WhenPhaseEnds(FGameplayTag PhaseTag, EPhaseTagMatchType MatchType, FHunterGamePhaseTagDynamicDelegate WhenPhaseEnd)
{
	const FHunterGamePhaseTagDelegate EndedDelegate = FHunterGamePhaseTagDelegate::CreateWeakLambda(WhenPhaseEnd.GetUObject(), [WhenPhaseEnd](const FGameplayTag& PhaseTag) {
...
void UHunterGamePhaseSubsystem::WhenPhaseStartsOrIsActive(FGameplayTag PhaseTag, EPhaseTagMatchType MatchType, const FHunterGamePhaseTagDelegate& WhenPhaseActive)
{
...
void UHunterGamePhaseSubsystem::WhenPhaseEnds(FGameplayTag PhaseTag, EPhaseTagMatchType MatchType, const FHunterGamePhaseTagDelegate& WhenPhaseEnd)
{
	FPhaseObserver Observer;
	Observer.PhaseTag = PhaseTag;
	Observer.MatchType = MatchType;
...
bool UHunterGamePhaseSubsystem::IsPhaseActive(const FGameplayTag& PhaseTag) const
{
	for (const auto& KVP : ActivePhaseMap)
	{
		const FHunterGamePhaseEntry& PhaseEntry = KVP.Value;
...
void UHunterGamePhaseSubsystem::OnBeginPhase(const UHunterGamePhaseAbility* PhaseAbility, const FGameplayAbilitySpecHandle PhaseAbilityHandle)
{
	...
	if (ensure(GameState_ASC))
	{
		...
		{
			const UHunterGamePhaseAbility* ActivePhaseAbility = CastChecked<UHunterGamePhaseAbility>(ActivePhase->Ability);
		...
		FHunterGamePhaseEntry& Entry = ActivePhaseMap.FindOrAdd(PhaseAbilityHandle);
...
void UHunterGamePhaseSubsystem::OnEndPhase(const UHunterGamePhaseAbility* PhaseAbility, const FGameplayAbilitySpecHandle PhaseAbilityHandle)
{
	const FGameplayTag EndedPhaseTag = PhaseAbility->GetGamePhaseTag();
	UE_LOG(LogHunterGamePhase, Log, TEXT("Ended Phase '%s' (%s)"), *EndedPhaseTag.ToString(), *GetNameSafe(PhaseAbility));

	const FHunterGamePhaseEntry& Entry = ActivePhaseMap.FindChecked(PhaseAbilityHandle);
...
bool UHunterGamePhaseSubsystem::FPhaseObserver::IsMatch(const FGameplayTag& ComparePhaseTag) const
{
	switch(MatchType)
	{
