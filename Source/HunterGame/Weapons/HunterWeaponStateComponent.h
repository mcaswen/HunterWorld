...
#include "HunterWeaponStateComponent.generated.h"
...
struct FHunterScreenSpaceHitLocation
{
...
struct FHunterServerSideHitMarkerBatch
{
	FHunterServerSideHitMarkerBatch() { }

	FHunterServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FHunterScreenSpaceHitLocation> Markers;
...
UCLASS()
class UHunterWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UHunterWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FHunterScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FHunterServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
