...
#include "HunterGameplayAbility_RangedWeapon.h"
#include "Weapons/HunterRangedWeaponInstance.h"
#include "Physics/HunterCollisionChannels.h"
#include "HunterLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/HunterWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/HunterGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterGameplayAbility_RangedWeapon)
...
namespace HunterConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("Hunter.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("Hunter.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("Hunter.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UHunterGameplayAbility_RangedWeapon::UHunterGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UHunterRangedWeaponInstance* UHunterGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UHunterRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UHunterGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogHunterAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UHunterRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UHunterGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UHunterGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UHunterGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Hunter_TraceChannel_Weapon;
}

FHitResult UHunterGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UHunterGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UHunterGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EHunterAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UHunterWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UHunterWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EHunterAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EHunterAbilityTargetingSource::CameraTowardsFocus) || (Source == EHunterAbilityTargetingSource::PawnTowardsFocus) || (Source == EHunterAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EHunterAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EHunterAbilityTargetingSource::WeaponForward) || (Source == EHunterAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EHunterAbilityTargetingSource::PawnTowardsFocus) || (Source == EHunterAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UHunterGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (HunterConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, HunterConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UHunterGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UHunterRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EHunterAbilityTargetingSource::CameraTowardsFocus);
		...
		if (HunterConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, HunterConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UHunterGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UHunterRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (HunterConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, HunterConsoleVariables::DrawBulletHitRadius, FColor::Red, false, HunterConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UHunterGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UHunterGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UHunterGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UHunterWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UHunterWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogHunterAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UHunterGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UHunterWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UHunterWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FHunterGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FHunterGameplayAbilityTargetData_SingleTargetHit();
