...
#include "HunterWeaponStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

UHunterWeaponStateComponent::UHunterWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UHunterWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (APawn* Pawn = GetPawn<APawn>())
	{
		...
		{
			if (UHunterRangedWeaponInstance* CurrentWeapon = Cast<UHunterRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(UHunterRangedWeaponInstance::StaticClass())))
			{
...
bool UHunterWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
...
bool UHunterWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UHunterWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FHunterServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FHunterScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void UHunterWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FHunterServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FHunterScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void UHunterWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UHunterWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UHunterWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
