...
#include "HunterRangedWeaponInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Hunter_Weapon_SteadyAimingCamera, "Hunter.Weapon.SteadyAimingCamera");

UHunterRangedWeaponInstance::UHunterRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UHunterRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UHunterRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UHunterRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UHunterRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UHunterRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UHunterRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UHunterRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UHunterRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UHunterRangedWeaponInstance::AddSpread()
{
...
float UHunterRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UHunterRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UHunterRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UHunterRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UHunterCameraComponent* CameraComponent = UHunterCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_Hunter_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
