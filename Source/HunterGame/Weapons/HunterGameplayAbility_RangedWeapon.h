...
#include "HunterGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UHunterRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EHunterAbilityTargetingSource : uint8
{
...
/**
 * UHunterGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UHunterGameplayAbility_RangedWeapon : public UHunterGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UHunterGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="Hunter|Ability")
	UHunterRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UHunterRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EHunterAbilityTargetingSource Source) const;
