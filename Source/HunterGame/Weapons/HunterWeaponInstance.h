...
#include "HunterWeaponInstance.generated.h"
...
/**
 * UHunterWeaponInstance
 *
 * A piece of equipment representing a weapon spawned and applied to a pawn
 */
UCLASS(MinimalAPI)
class UHunterWeaponInstance : public UHunterEquipmentInstance
{
	GENERATED_BODY()

public:
	UE_API UHunterWeaponInstance(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	...
	FHunterAnimLayerSelectionSet EquippedAnimSet;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Animation)
	FHunterAnimLayerSelectionSet UneuippedAnimSet;
