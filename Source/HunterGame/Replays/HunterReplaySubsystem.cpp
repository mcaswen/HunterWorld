...
#include "HunterReplaySubsystem.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterReplaySubsystem)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Platform_Trait_ReplaySupport, "Platform.Trait.ReplaySupport");

UHunterReplaySubsystem::UHunterReplaySubsystem()
{
}

bool UHunterReplaySubsystem::DoesPlatformSupportReplays()
{
...
FGameplayTag UHunterReplaySubsystem::GetPlatformSupportTraitTag()
{
	return TAG_Platform_Trait_ReplaySupport.GetTag();
}

void UHunterReplaySubsystem::PlayReplay(UHunterReplayListEntry* Replay)
{
	if (Replay != nullptr)
	{
...
void UHunterReplaySubsystem::RecordClientReplay(APlayerController* PlayerController)
{
	if (ensure(DoesPlatformSupportReplays() && PlayerController))
	{
		FText FriendlyNameText = FText::Format(NSLOCTEXT("Hunter", "HunterReplayName_Format", "Client Replay {0}"), FText::AsDateTime(FDateTime::UtcNow(), EDateTimeStyle::Short, EDateTimeStyle::Short));
		GetGameInstance()->StartRecordingReplay(FString(), FriendlyNameText.ToString());

		if (UHunterLocalPlayer* HunterLocalPlayer = Cast<UHunterLocalPlayer>(PlayerController->GetLocalPlayer()))
		{
			// Start a cleanup of existing saved streams
			int32 NumToKeep = HunterLocalPlayer->GetLocalSettings()->GetNumberOfReplaysToKeep();
			CleanupLocalReplays(HunterLocalPlayer, NumToKeep);
		}
	}
}

void UHunterReplaySubsystem::CleanupLocalReplays(ULocalPlayer* LocalPlayer, int32 NumReplaysToKeep)
{
...
void UHunterReplaySubsystem::OnEnumerateStreamsCompleteForDelete(const FEnumerateStreamsResult& Result)
{
	...
	if (StreamsToDelete.Num() > DeletingReplaysNumberToKeep)
	{
		...
		UE_LOG(LogHunter, Log, TEXT("HunterReplaySubsystem asked to delete replay %s"), *ReplayName);
...
void UHunterReplaySubsystem::OnDeleteReplay(const FDeleteFinishedStreamResult& DeleteResult)
{
	if (!CurrentReplayStreamer.IsValid() || !IsValid(LocalPlayerDeletingReplays))
	{
		// Lost context, don't do anything
	...
	else
	{
		...
		UE_LOG(LogHunter, Warning, TEXT("Failed to delete replay with error %d!"), (int32)DeleteResult.Result);
...
void UHunterReplaySubsystem::SeekInActiveReplay(float TimeInSeconds)
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float UHunterReplaySubsystem::GetReplayLengthInSeconds() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float UHunterReplaySubsystem::GetReplayCurrentTime() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
UDemoNetDriver* UHunterReplaySubsystem::GetDemoDriver() const
{
