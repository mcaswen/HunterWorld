...
#include "HunterReplaySubsystem.generated.h"
...
class UHunterReplayListEntry : public UObject
{
	GENERATED_BODY()
...
UCLASS(MinimalAPI, BlueprintType)
class UHunterReplayList : public UObject
{
	GENERATED_BODY()

public:
	UPROPERTY(BlueprintReadWrite, Category=Replays)
	TArray<TObjectPtr<UHunterReplayListEntry>> Results;
};

/** Subsystem to handle recording/loading replays */
UCLASS(MinimalAPI)
class UHunterReplaySubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

public:
	UE_API UHunterReplaySubsystem();
	...
	UE_API void PlayReplay(UHunterReplayListEntry* Replay);
