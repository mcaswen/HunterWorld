...
class IHunterTeamAgentInterface;
class UHunterTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UHunterTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements IHunterTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UHunterTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UHunterTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<IHunterTeamAgentInterface> TeamInterfacePtr;
