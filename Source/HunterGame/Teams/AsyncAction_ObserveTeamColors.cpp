...
#include "Teams/HunterTeamStatics.h"
...
UAsyncAction_ObserveTeamColors* UAsyncAction_ObserveTeamColors::ObserveTeamColors(UObject* TeamAgent)
{
	...
	if (TeamAgent != nullptr)
	{
		...
		Action->TeamInterfacePtr = TWeakInterfacePtr<IHunterTeamAgentInterface>(TeamAgent);
...
void UAsyncAction_ObserveTeamColors::SetReadyToDestroy()
{
	...
	if (IHunterTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
...
void UAsyncAction_ObserveTeamColors::Activate()
{
	...
	UHunterTeamDisplayAsset* CurrentDisplayAsset = nullptr;
	...
	if (IHunterTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
		if (UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull))
		{
			...
			CurrentDisplayAsset = UHunterTeamStatics::GetTeamDisplayAsset(World, CurrentTeamIndex);
...
void UAsyncAction_ObserveTeamColors::BroadcastChange(int32 NewTeam, const UHunterTeamDisplayAsset* DisplayAsset)
{
	UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull);
	UHunterTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<UHunterTeamSubsystem>(World);
...
{
	UHunterTeamDisplayAsset* DisplayAsset = UHunterTeamStatics::GetTeamDisplayAsset(TeamAgent, NewTeam);
	BroadcastChange(NewTeam, DisplayAsset);
}

void UAsyncAction_ObserveTeamColors::OnDisplayAssetChanged(const UHunterTeamDisplayAsset* DisplayAsset)
{
