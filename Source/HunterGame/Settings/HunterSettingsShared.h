...
#include "HunterSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EHunterAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UHunterSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UHunterSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UHunterSettingsShared, FOnSettingChangedEvent, UHunterSettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	UHunterSettingsShared();
	...
	static UHunterSettingsShared* CreateTemporarySettings(const UHunterLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UHunterSettingsShared* LoadOrCreateSettings(const UHunterLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UHunterSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UHunterLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EHunterAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EHunterAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EHunterAllowBackgroundAudioSetting AllowAudioInBackground = EHunterAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EHunterGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EHunterGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EHunterGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EHunterGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EHunterGamepadSensitivity GamepadLookSensitivityPreset = EHunterGamepadSensitivity::Normal;
	UPROPERTY()
	EHunterGamepadSensitivity GamepadTargetingSensitivityPreset = EHunterGamepadSensitivity::Normal;
