...
#include "HunterSafeZoneEditor.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterSafeZoneEditor)
...
UHunterSafeZoneEditor::UHunterSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UHunterSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UHunterSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UHunterSettingsLocal::Get()->GetSafeZone());
	
	Button_Done->OnClicked().AddUObject(this, &UHunterSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UHunterSafeZoneEditor::HandleBackClicked);
	}
}

bool UHunterSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UHunterSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UHunterSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UHunterSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UHunterSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UHunterSettingsLocal::Get()->GetSafeZone());
}

void UHunterSafeZoneEditor::HandleDoneClicked()
{
	...
	{
		UHunterSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
