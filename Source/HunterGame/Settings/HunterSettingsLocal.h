...
#include "HunterSettingsLocal.generated.h"
...
USTRUCT()
struct FHunterScalabilitySnapshot
{
	GENERATED_BODY()

	FHunterScalabilitySnapshot();
...
/**
 * UHunterSettingsLocal
 */
UCLASS()
class UHunterSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	static UHunterSettingsLocal* Get();
...
public:
	...
	EHunterStatDisplayMode GetPerfStatDisplayState(EHunterDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EHunterDisplayablePerformanceStat Stat, EHunterStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UHunterSettingsLocal, FPerfStatSettingsChanged);
	...
	DECLARE_EVENT(UHunterSettingsLocal, FLatencyFlashInidicatorSettingChanged);
	UFUNCTION()
	...
	DECLARE_EVENT(UHunterSettingsLocal, FLatencyStatEnabledSettingChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EHunterDisplayablePerformanceStat, EHunterStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FHunterScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FHunterScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UHunterSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
