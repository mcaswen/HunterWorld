...
#include "Audio/HunterAudioSettings.h"
#include "Audio/HunterAudioMixEffectsSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HunterSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Hunter.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Hunter.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Hunter.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Hunter.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Hunter.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Hunter.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Hunter.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Hunter.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Hunter.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Hunter.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FHunterScalabilitySnapshot::FHunterScalabilitySnapshot()
{
...
namespace HunterSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FHunterScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
		...
		Mode = FHunterScalabilitySnapshot();
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UHunterSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UHunterSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UHunterSettingsLocal::UHunterSettingsLocal()
{
	...
	bEnableScalabilitySettings = UHunterPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings;

	SetToDefaults();
}

void UHunterSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
	...
	bEnableLatencyTrackingStats = UHunterSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void UHunterSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHunterFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	HunterSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UHunterSettingsLocal::ResetToCurrentSettings()
{
...
void UHunterSettingsLocal::BeginDestroy()
{
...
UHunterSettingsLocal* UHunterSettingsLocal::Get()
{
	return GEngine ? CastChecked<UHunterSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UHunterSettingsLocal::ConfirmVideoMode()
{
...
float UHunterSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EHunterFramePacingMode::DesktopStyle)
	{
...
int32 UHunterSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return HunterSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UHunterSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FHunterScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UHunterSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UHunterSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UHunterSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UHunterSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UHunterSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UHunterSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EHunterStatDisplayMode UHunterSettingsLocal::GetPerfStatDisplayState(EHunterDisplayablePerformanceStat Stat) const
{
	if (const EHunterStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EHunterStatDisplayMode::Hidden;
	}
}

void UHunterSettingsLocal::SetPerfStatDisplayState(EHunterDisplayablePerformanceStat Stat, EHunterStatDisplayMode DisplayMode)
{
	if (DisplayMode == EHunterStatDisplayMode::Hidden)
	{
...
bool UHunterSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void UHunterSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void UHunterSettingsLocal::SetEnableLatencyTrackingStats(const bool bNewVal)
{
...
void UHunterSettingsLocal::ApplyLatencyTrackingStatSetting()
{
...
bool UHunterSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float UHunterSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UHunterSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UHunterSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UHunterSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UHunterSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UHunterSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UHunterSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UHunterSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UHunterSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UHunterSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UHunterSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UHunterSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UHunterSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UHunterSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UHunterSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UHunterSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return HunterSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UHunterSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return HunterSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UHunterSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UHunterSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EHunterFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return HunterSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UHunterSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHunterFramePacingMode::MobileStyle)
	{
...
void UHunterSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UHunterSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHunterFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = HunterSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UHunterSettingsLocal::ClampMobileQuality()
{
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHunterFramePacingMode::MobileStyle)
	{
...
void UHunterSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = HunterSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UHunterSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = HunterSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = HunterSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UHunterSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UHunterSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UHunterSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UHunterSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UHunterSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = HunterSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UHunterSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UHunterSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UHunterAudioMixEffectsSubsystem* HunterAudioMixEffectsSubsystem = World->GetSubsystem<UHunterAudioMixEffectsSubsystem>())
			{
				HunterAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UHunterSettingsLocal::CanRunAutoBenchmark() const
{
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UHunterSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UHunterSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UHunterSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UHunterSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UHunterSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UHunterSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UHunterSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UHunterSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UHunterSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UHunterSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UHunterSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UHunterSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UHunterSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UHunterSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UHunterSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UHunterSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UHunterSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UHunterSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UHunterSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EHunterFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = HunterSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UHunterSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UHunterSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UHunterSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UHunterAudioSettings* HunterAudioSettings = GetDefault<UHunterAudioSettings>())
			{
				...
				if (UObject* ObjPath = HunterAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from Hunter Audio Settings."));
					}
				}

				if (UObject* ObjPath = HunterAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from Hunter Audio Settings."));
					}
				}

				if (UObject* ObjPath = HunterAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from Hunter Audio Settings."));
					}
				}

				if (UObject* ObjPath = HunterAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from Hunter Audio Settings."));
					}
				}

				if (UObject* ObjPath = HunterAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from Hunter Audio Settings."));
					}
				}

				if (UObject* ObjPath = HunterAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Hunter Audio Settings."));
					}
				}
			}
		}
...
void UHunterSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UHunterSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UHunterPlatformSpecificRenderingSettings* PlatformSettings = UHunterPlatformSpecificRenderingSettings::Get();
	const TArray<FHunterQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FHunterQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UHunterPlatformEmulationSettings* Settings = GetDefault<UHunterPlatformEmulationSettings>();
	...
	{
	case EHunterFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EHunterFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EHunterFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UHunterSettingsLocal::UpdateConsoleFramePacing()
{
...
void UHunterSettingsLocal::UpdateDesktopFramePacing()
{
...
void UHunterSettingsLocal::UpdateMobileFramePacing()
{
...
void UHunterSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
