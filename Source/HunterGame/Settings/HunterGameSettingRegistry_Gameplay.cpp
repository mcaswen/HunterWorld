...
#include "CustomSettings/HunterSettingValueDiscrete_Language.h"
...
UGameSettingCollection* UHunterGameSettingRegistry::InitializeGameplaySettings(UHunterLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			UHunterSettingValueDiscrete_Language* Setting = NewObject<UHunterSettingValueDiscrete_Language>();
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UHunterSettingsLocal>()->ShouldAutoRecordReplays());

			Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
			Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(UHunterReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UHunterSettingsLocal>()->GetNumberOfReplaysToKeep());
