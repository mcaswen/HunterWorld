...
#include "Private/AssetTypeActions_HunterContextEffectsLibrary.h"
#include "ToolMenu.h"
#include "ToolMenus.h"
...
DEFINE_LOG_CATEGORY(LogHunterEditor);
...
{
	return HasNoPlayWorld() && !GetDefault<UHunterDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<UHunterDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
 ...
 * FHunterEditorModule
 */
class FHunterEditorModule : public FDefaultGameModuleImpl
{
	typedef FHunterEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FHunterEditorModule::ModulesChangedCallback);
		...
		// Register the Context Effects Library asset type actions.
		{
			...
			TSharedRef<FAssetTypeActions_HunterContextEffectsLibrary> AssetAction = MakeShared<FAssetTypeActions_HunterContextEffectsLibrary>();
			HunterContextEffectsLibraryAssetAction = AssetAction;
	...
	{
		UHunterExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<UHunterExperienceManager>();
	...
	virtual void ShutdownModule() override
	{
		// Unregister the Context Effects Library asset type actions.
		{
			...
			TSharedPtr<IAssetTypeActions> AssetAction = HunterContextEffectsLibraryAssetAction.Pin();
...
private:
	TWeakPtr<IAssetTypeActions> HunterContextEffectsLibraryAssetAction;
	FDelegateHandle ToolMenusHandle;
};

IMPLEMENT_MODULE(FHunterEditorModule, HunterEditor);
